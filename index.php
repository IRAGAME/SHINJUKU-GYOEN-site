...
<style>
        ...
        #chatFab svg { width: 28px; height: 28px; fill: #fff; }
        #chatWidget { position: fixed; bottom: 92px; left: 24px; z-index: 62; width: 380px; max-width: calc(100vw - 32px); height: 520px; max-height: calc(100vh - 140px); border-radius: 16px; overflow: hidden; display: flex; flex-direction: column; background: #fff; box-shadow: 0 12px 40px rgba(0,0,0,.18); opacity: 0; transform: translateY(12px) scale(0.97); pointer-events: none; transition: opacity .25s ease, transform .25s ease; }
        ...
        .chat-messages { flex: 1; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 6px; background: #ECE5DD; }
        .chat-msg { max-width: 80%; padding: 8px 12px; border-radius: 12px; font-family: 'Manrope', sans-serif; font-size: 14px; line-height: 1.45; word-break: break-word; position: relative; }
        ...
        .chat-msg.admin .time { color: rgba(0,0,0,.35); }
        ...
        .chat-typing { align-self: flex-start; background: #fff; padding: 10px 16px; border-radius: 12px; box-shadow: 0 1px 1px rgba(0,0,0,.06); display: none; }
        .chat-typing span { display: inline-block; width: 6px; height: 6px; border-radius: 9999px; background: #999; animation: typing-dot .9s ease-in-out infinite; }
        ...
        body.offline #chatFab, body.offline #chatWidget { display: none !important; }
...
<div class="status" id="chatStatus">En ligne</div>
</div>
...
<h3>Besoin d'aide ?</h3>
<p>Écrivez-nous, nous répondons généralement dans les plus brefs délais.</p>
...
<input type="tel" id="chatPhone" placeholder="Votre WhatsApp (optionnel, ex: +257...)" maxlength="20"/>
...
(function () {
    ...
    chatFab.addEventListener('click', function () {
        ...
        if (chatWidget.classList.contains('open') && conversationId) {
            chatInput.focus();
        }
    });
    ...
    function startConversation() {
        ...
        .catch(function () {
            chatInitError.textContent = 'Erreur réseau. Vérifiez votre connexion.';
            chatInitError.style.display = 'block';
            chatStart.disabled = false;
            chatStart.textContent = 'Commencer la conversation';
        });
    ...
    // Polling : vérifier les nouveaux messages toutes les 3 secondes
    function startPolling() {
        if (pollTimer) return;
        lastMessageAt = new Date().toISOString().replace('T', ' ').substring(0, 19);
        pollTimer = setInterval(pollMessages, 3000);
    }

    function pollMessages() {
        if (!conversationId) return;
        fetch(API + 'messages/poll/' + conversationId + '?since=' + encodeURIComponent(lastMessageAt))
        .then(function (r) { return r.json(); })
        .then(function (res) {
            if (res.success && res.data && res.data.length > 0) {
                res.data.forEach(function (msg) {
                    addMessage(msg.sender, msg.body, msg.created_at);
                });
                // Afficher le badge si le widget est fermé
                if (!chatWidget.classList.contains('open')) {
                    chatBadge.textContent = res.data.length;
